...
const Index = () => {
  ...
  useEffect(() => {
    ...
    const distance = Math.sqrt(Math.pow(mousePosition.x - buttonCenterX, 2) + Math.pow(mousePosition.y - buttonCenterY, 2));
    if (distance < 70) {
      teleportNoButton();
    }
  ...
  const teleportNoButton = () => {
    ...
    do {
      newX = Math.max(10, Math.random() * maxX);
      newY = Math.max(10, Math.random() * maxY);
      const distanceToYes = Math.sqrt(Math.pow(newX + buttonWidth / 2 - (yesRect.left + yesRect.width / 2), 2) + Math.pow(newY + buttonHeight / 2 - (yesRect.top + yesRect.height / 2), 2));
  ...
  if (accepted) {
    return <div className="min-h-screen bg-primary flex items-center justify-center p-4 transition-all duration-150">
  ...
  return <div ref={containerRef} className="min-h-screen bg-background flex items-center justify-center p-4 relative overflow-hidden">
      <div className="border-thick border-primary bg-background p-12 max-w-2xl w-full relative">
        <h1 className="text-2xl font-bold text-foreground uppercase tracking-tight text-center mb-12">GREAT CHOICE :)</h1>
        
        <div className="flex justify-center gap-8 relative h-32">
          <Button ref={yesButtonRef} onClick={handleYesClick} variant="brutalist-primary" className="font-bold uppercase text-sm tracking-wide">
            YES
          </Button>

          <Button ref={noButtonRef} onMouseEnter={handleNoButtonHover} variant="brutalist-secondary" className="font-bold uppercase text-sm tracking-wide !bg-white !text-black !border-4 !border-black" style={hasBeenHovered ? {
          position: 'fixed',
